...
exports.SqlCompilerImpl = void 0;
const debug_1 = __importDefault(require("debug"));
const log = (0, debug_1.default)('queryleaf:compiler');
/**
...
class SqlCompilerImpl {
    ...
    compileInsert(ast) {
        ...
        const documents = ast.values.map((valueList) => {
            const document = {};
    ...
    convertValue(value) {
        if (typeof value === 'object') {
            ...
            if (value.type === 'expr_list' && Array.isArray(value.value)) {
                return value.value.map((item) => this.convertValue(item));
            }
     ...
     */
    handleNestedFieldReferences(ast) {
     ...
     */
    processWhereClauseForNestedFields(where) {
        if (!where)
            return;
        ...
        if (where.type === 'binary_expr') {
            ...
            if (where.left && where.left.type === 'column_ref') {
                ...
                if (where.left.column && where.left.column.includes('.')) {
                    ...
                    log('Converting table.column to nested path:', `${where.left.table}.${where.left.column}`);
     ...
     */
    convertGroupBy(groupby, columns) {
     ...
     */
    createAggregatePipeline(command) {
        const pipeline = [];
     ...
     */
    needsAggregationProjection(projection) {
        // Check if any value is a string that starts with $
        return Object.values(projection).some(value => typeof value === 'string' && value.startsWith('$'));
    }
    /**
     * Convert a MongoDB projection to $project format used in aggregation pipeline
     */
    convertToAggregationProjection(projection) {
        const result = {};
    ...
    convertJoins(from, where) {
        ...
        for (let i = 1; i < from.length; i++) {
            ...
            const joinCond = joinConditions.find(cond => (cond.leftTable === mainTable && cond.rightTable === joinedTable) ||
     ...
     * Extract join conditions from the WHERE clause
     */
    extractJoinConditions(where, tables) {
        if (!where) {
            return [];
        }
...
exports.SqlCompilerImpl = SqlCompilerImpl;
