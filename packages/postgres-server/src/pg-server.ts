...
class PostgresServer {
  ...
  constructor(
    ...
    this.server.listen(options.port, options.host, () => {
      debug(`PostgreSQL-compatible server running at ${options.host}:${options.port}`);
      debug(`Connected to MongoDB (${dbName}) at ${mongoClient.options.hosts?.join(',')}`);
    });
...
async function main(): Promise<void> {
  const { uri, db, port, host, maxConnections } = argv;
  ...
  try {
    ...
    new PostgresServer(mongoClient, db as string, {
      port: port as number,
      host: host as string,
